...
export default window.yufp = {
  ...
  util: {
    ...
    genTree: function (data, attr) {
      ...
      if (attr.root == null || attr.root == undefined || attr.root == '') {
        ...
        root.label =
          root[attr.label] === undefined ? root.label : root[attr.label]
      } else {
        for (var i in data) {
    ...
    addTokenInfo: function (url) {
      ...
      if (!url.indexOf(token) > -1) {
        _url =
          url + (url.indexOf('?') > -1 ? '&' : '?') + token + getAccessToken()
      }
      return _url
    },
    dateFormat
  ...
  service: {
    ...
    deepTraverseMsg: function (res) {
      ...
      if ((res.message || res.msg) && res.code) {
        res[res.message ? 'message' : 'msg'] =
          _this.handleMsg(res) || res.message || res.msg || ''
      }
    ...
    handleMsg: function (data) {
      ...
      if (code && String(code).indexOf('|') != -1) {
        ...
        transalteMsg =
          yufp.language.getLocaleText('message.' + code, data.i18nData) ||
          message
      }
      return transalteMsg
    },
  ...
  language: {
    ...
    getLocaleText: function (key, ops, count) {
      return this.resetTemplate(
        this.getValueByPath(
          window[
            'lang' +
              this.language.substr(0, 1).toLocaleUpperCase() +
              this.language.substring(1)
          ],
          key
        ),
        ops,
    ...
    resetTemplate: function (template, ops, count) {
      ...
      var temp =
        ops && template
          ? template.replace(/{\w*}/g, function (node, key, index) {
              return ops[num++] || ''
            })
          : template
