...
class Ajax {
  ...
  constructor(args) {
    ...
    if (!xhr) {
      core.logger.error(
        'core.ajax: Your browser not support AJAX XMLHttpRequest.'
      );
      return;
    }
    var setting = {};
  ...
  processData(args, method) {
    ...
    try {
      ...
      core.logger.error(
        'core.ajax: Request params format error；' + ex.Message,
        ex
      );
    }
    return reqData;
    ...
    xhr.onreadystatechange = function() {
      try {
        ...
        if (
          ...
          var rspData =
            xhr.response == undefined ? xhr.responseText : xhr.response;
        ...
        for (var key in data) {
          ...
          if (valType == 'array') {
            for (var i = 0; i < value.length; i++) {
              var item =
                key + '=' + (!encode ? value : encodeURIComponent(value[i]));
              if (res.length > 0) {
                res += '&';
              }
              res += item;
            ...
            var item =
              key + '=' + (!encode ? value : encodeURIComponent(value));
            if (res.length > 0) {
              res += '&';
            }
            res += item;
...
class Service {
  ...
  deepTraverseMsg = function(res) {
    if ((res.message || res.msg) && res.code) {
      res[res.message ? 'message' : 'msg'] =
        this.handleMsg(res) || res.message || res.msg || '';
    }
  ...
  handleMsg(data) {
    ...
    if (code && String(code).indexOf('|') != -1) {
      ...
      for (var len = code.length; i < len; ++i) {
        ...
        res =
          getI18nMessage('message.' + curCode, data.i18nData, countMsg) ||
          message[i] ||
          '';
      ...
      transalteMsg =
        getI18nMessage('message.' + code, data.i18nData) || message;
    }
    return transalteMsg;
  }
