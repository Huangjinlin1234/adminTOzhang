...
const frameConfig = {
  ...
  baseFrameOptions: {
    ...
    themeTool: [
      ...
      {
        show: true,
        text: getI18nMessage('component.model'),
        id: 'sizeModel',
        extend: true
      } // extend标注是否显示自动滑块
    ],
    ...
    themesList: [
      // 默认是紫色,皮肤id需要和主题中皮肤目录相同对应
      {
        id: 'zrcbank',
        color: '#2877FF',
        name: getI18nMessage('component.skinBlue'),
        checked: true
      },
      {
        id: 'blue',
        color: '#2877FF',
        name:
          getI18nMessage('component.skinBlue') +
          '（' +
          getI18nMessage('component.mr') +
          '）'
      },
      {
        id: 'orange',
        color: '#FB8D12',
        name: getI18nMessage('component.skinOrange')
      },
      {
        id: 'purple',
        color: '#5557b9',
        name: getI18nMessage('component.skinPurple')
      }
    ],
    // 字号
    // fontSizeList: [
    ...
    appOptions: [
      ...
      {
        applicationId: '1',
        applicationName: getI18nMessage('component.ptglxt')
      },
    ...
    sysTools: [
      ...
      {
        show: true,
        text: getI18nMessage('component.message'),
        icon: 'yu-icon-message',
        className: '',
        id: 'message',
        badgeDot: true
      },
      {
        show: false,
        text: getI18nMessage('component.themes'),
        icon: 'yu-icon-theme',
        className: '',
        id: 'themes'
      },
          ...
          click: function (event) {
            ...
            event.target.setAttribute(
              ...
              screenfull.isFullscreen
                ? className.replace('-unfull-', '-full-')
                : className.replace('-full-', '-unfull-')
            )
            screenfull.toggle()
          }
        }
      },
      {
        ...
        event: {
          click: function (item, menuModel) {
            // 隐藏工具栏
            const toolbar =
              document.getElementsByClassName('yu-frame-top-bar')[0]
            toolbar.setAttribute(
              'class',
              'yu-frame-top-bar yu-frame-top-bar-pickup'
            )
            if (menuModel.id === 'left' || menuModel.id === 'right') {
              ...
              menuModel.id === 'topTile' ||
              menuModel.id === 'topTree'
            ) {
              // 设置菜单高度样式
              const frameMenu =
                document.getElementsByClassName('yu-frame-menu')[0]
              frameMenu.setAttribute(
                'class',
                'yu-frame-menu yu-frame-menu-pickup'
              )
